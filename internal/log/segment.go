...
For example, when the log appends a record to the active segment, the segment needs to write the data
to its store and add a new entry in the index. Similarly, for reads, the segment needs to look up the entry from
the index and then fetch the data from the store.
*/
...
func newSegment(dir string, baseOffset uint64, c Config) (*segment, error) {
	...
	if off, _, err := s.index.Read(-1); err != nil {
		...
		// f the index has at least one entry, then that means the offset of
